...
-- This code is only the demo for testing i2c interface.
-- For the real oled display use the oled module.
...
function write_reg(dev_addr, reg_addr, reg_val)
     if i2c.write(id, dev_addr, reg_addr, reg_val) < 0 then
...
-- initialize i2c
if id == 0 then
	i2c.setup(id,0,1) -- hardware i2c
else
	i2c.setup(id,sda,scl) -- hardware i2c
end
