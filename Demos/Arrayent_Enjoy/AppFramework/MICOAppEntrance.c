/**
  ...
  * @version V1.0.0
  * @date    05-May-2014
  * @brief   MiCO application entrance, add user application functons and threads.
  ******************************************************************************
  * @attention
  *
...
void appRestoreDefault_callback(void * const user_config_data, uint32_t size)
{
  ...
  MiCOArrayentRestoreDefault(&(appConfig->arrayentConfig));
}
...
int application_start(void)
{
  ...
  require_noerr_action(err, exit, 
                       app_log("ERROR: mico_system_notify_register (mico_notify_WIFI_CONNECT_FAILED) failed!") );
  
  /* we moved this function before wifi configuration in function mico_system_init */
...
//#endif
  ...
  app_log("start cloud service: FogCloud.");
  err = MiCOStartFogCloudService( app_context );
  require_noerr_action( err, exit, app_log("ERROR: Unable to start FogCloud service.") );
#elif (MICO_CLOUD_TYPE == CLOUD_ARRAYENT)
  app_log("start cloud service: Arrayent.");
  err = MiCOStartArrayentService( app_context );
  require_noerr_action( err, exit, app_log("ERROR: Unable to start FogCloud service.") );
#elif (MICO_CLOUD_TYPE == CLOUD_ALINK)
  app_log("start cloud service: Alink.");
#elif (MICO_CLOUD_TYPE == CLOUD_DISABLED)
  app_log("Cloud service: disabled.");
#else
